#!/usr/bin/env python3
"""
Direct import of Roboto's memory data into the database
This version connects directly to the database without importing the app
"""
...
# Define minimal models needed for import
class User(Base):
    __tablename__ = 'users'
...
class UserData(Base):
    ...
    memory_system_data = Column(JSON, default=dict)
    current_emotion = Column(String(50), default='curious')
...
def get_or_create_roboto_user():
    ...
    roboto_user = session.query(User).filter_by(username='roboto_system').first()
    if not roboto_user:
        roboto_user = User(
...
def import_current_memory(roboto_user):
    ...
    user_data = session.query(UserData).filter_by(user_id=roboto_user.id).first()
    if not user_data:
    ...
    batch_size = 100
    for i in range(0, len(episodic_memories), batch_size):
        batch = episodic_memories[i:i + batch_size]
        ...
        for memory in batch:
            ...
            existing_memory = session.query(MemoryEntry).filter_by(
        ...
        # Commit batch
        session.commit()
        print(f"  Processed batch {i//batch_size + 1}/{(len(episodic_memories) + batch_size - 1)//batch_size}")
...
def import_backup_memory(roboto_user):
    ...
    # Process recent chat history into memories
    chat_history = backup_data.get('chat_history', [])
    if chat_history:
        print(f"Processing last 50 of {len(chat_history)} chat history entries...")
        ...
        for i, chat in enumerate(chat_history[-50:]):  # Process last 50 chat entries
            if isinstance(chat, dict):
                ...
                if user_msg and assistant_msg:
                    ...
                    existing = session.query(MemoryEntry).filter_by(
                        user_id=roboto_user.id,
                        memory_id=memory_id
                    ).first()
...
def main():
    ...
    try:
        ...
        total_memories = session.query(MemoryEntry).filter_by(user_id=roboto_user.id).count()
        episodic_count = session.query(MemoryEntry).filter_by(
            user_id=roboto_user.id,
            memory_type='episodic'
        ).count()
        semantic_count = session.query(MemoryEntry).filter_by(
            user_id=roboto_user.id,
